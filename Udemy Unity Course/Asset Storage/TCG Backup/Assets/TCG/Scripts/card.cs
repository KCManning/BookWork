...
public class card : MonoBehaviour {
    ...
    public static explicit operator GameObject(card v)
    {
						...
						for (int i = 0; i < DiscardCost; i++) 
								{
									...
									if (Player.ActionCancelled) { Debug.Log("action cancelled"); yield return false; }
								}
		...
		if (NoTargetEffects.Contains(effect.type))
			...
			Debug.Log("no target effect");
				...
				case 201:	//param0-param1 random enemy creatures
						...
						int number_of_creatures = UnityEngine.Random.Range (number_from, number_to + 1); //+1 because upper int is not included
